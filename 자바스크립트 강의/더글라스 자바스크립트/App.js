...
//기본값 설정 방법. 앞에 값이 없을 경우에는 뒤에있는걸로 default설정.

var middle = stooge["middle"] || "(none)";
var status = flight.status || "unknown";   
var first = stooge["first"] || "none";

//객체의 속성을 아래와 같이 추가해줄 수 있다.
...
// 객체는 참조 방식으로 전달되며 새로 생성하는게 아니라 기존에 있는 주소를 그대로 넘겨주는 방식 >> 값을 변경하면 참조한 값도 같이 변경됨
...
//프로토 타입
/*
    객체 리터럴로 생성되는 모든 객체는 표준 객체인 Objec의 속성인 prototype(Object.prototype) 객체에 연결된다.
*/
...
//재귀적호출
//재귀 함수는 직접 또는 간접적으로 자신을 호출하는 함수이다. 재귀적 호출은 어떤 문제가 유사한 하위 문제로 나뉘어지고 각가의 문제를 같은 해결 방법으로 처리할 수 있을때 사용하는 강력한 프로그래밍 기법이다.
